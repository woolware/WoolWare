...
import wool.ware.client.arraylist.ArrayMember;
...
import wool.ware.client.gui.tab.TabGUI;
...
import wool.ware.client.utils.value.impl.*;
...
public class HUD extends Module {
    ...
    private BooleanValue xyz = new BooleanValue("XYZ", "Coordinates", false);
    private BooleanValue fps = new BooleanValue("FPS", "FPS Display", false);
    private BooleanValue watermaker = new BooleanValue("Watermark", "Displays Watermark", true);
    private BooleanValue advertise = new BooleanValue("Advertise", "Displays WoolWare Website Link", true);
    private BooleanValue ping = new BooleanValue("Ping", "Show Ping", false);
    private BooleanValue armor = new BooleanValue("Armor", "Show Armor", false);
    private BooleanValue healingpotions = new BooleanValue("Healing Potions", "Show Healing Pots", false);
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (!animations.isEnabled()) {
            float posY = 3;
            final ArrayList<Module> mods = new ArrayList<>(Wool.INSTANCE.getModuleManager().getModuleMap().values());
            ...
            for (Module module : mods) {
                ...
                switch (MODECOLORS.getValue()) {
                    ...
                    case CLIENTCOLOR:
                        ...
                        break;
                    case WAVE:
                        color = color((int) (posY / (font.isEnabled() ? fontValue.getValue().getHeight() + 6 : getMc().fontRendererObj.FONT_HEIGHT + 4)), (int) Wool.INSTANCE.getModuleManager().getMap().values().stream().filter(module1 -> module1.isEnabled() && !module1.isHidden()).count());
                        break;
                }
        ...
        if (ping.isEnabled()) {
            if (font.isEnabled())
                fontValue.getValue().drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toLowerCase() : ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer))), event.getScaledResolution().getScaledWidth() - 2 - fontValue.getValue().getStringWidth(MODECASE.getValue() == casemode.UPPER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toLowerCase() : ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)))), event.getScaledResolution().getScaledHeight() - (getMc().ingameGUI.getChatGUI().getChatOpen() ? 36 : 24), colorWaterMark);
            else
                getMc().fontRendererObj.drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toLowerCase() : ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer))), event.getScaledResolution().getScaledWidth() - 2 - getMc().fontRendererObj.getStringWidth(MODECASE.getValue() == casemode.UPPER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)).toLowerCase() : ("Ping: " + (getMc().isSingleplayer() ? "0" : getMc().getCurrentServerData().pingToServer)))), event.getScaledResolution().getScaledHeight() - (getMc().ingameGUI.getChatGUI().getChatOpen() ? 36 : 24), colorWaterMark);
        }
        if (fps.isEnabled()) {
            if (font.isEnabled())
                fontValue.getValue().drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("F" + "PS: " + Minecraft.getDebugFPS()).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("F" + "PS: " + Minecraft.getDebugFPS()).toLowerCase() : ("F" + "PS: " + Minecraft.getDebugFPS())), 3, tabgui.isEnabled() ? 100 : 15, colorWaterMark);
            else
                getMc().fontRendererObj.drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("F" + "PS: " + Minecraft.getDebugFPS()).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("F" + "PS: " + Minecraft.getDebugFPS()).toLowerCase() : ("F" + "PS: " + Minecraft.getDebugFPS())), 3, tabgui.isEnabled() ? 100 : 15, colorWaterMark);
        }
        
        if (advertise.isEnabled()) {
            if (font.isEnabled())
                fontValue.getValue().drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? (("https://woolware.github.io".toLowerCase())) : (MODECASE.getValue() == casemode.LOWER ? (("https://woolware.github.io".toLowerCase())) : (("https://woolware.github.io".toLowerCase()))), 3, tabgui.isEnabled() ? 100 : 15, colorWaterMark);
            else
                getMc().fontRendererObj.drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? (("https://woolware.github.io".toUpperCase())) : (MODECASE.getValue() == casemode.LOWER ? ("https://woolware.github.io".toUpperCase()) : (("https://woolware.github.io".toUpperCase()))), 3, tabgui.isEnabled() ? 100 : 15, colorWaterMark);
            }
        ...
        if (healingpotions.isEnabled()) {
            if (font.isEnabled())
                fontValue.getValue().drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("Healing Pots: " + getPotions()).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Healing Pots: " + getPotions()).toLowerCase() : ("Healing Pots: " + getPotions())), 3, event.getScaledResolution().getScaledHeight() - (getMc().ingameGUI.getChatGUI().getChatOpen() ? 24 : 12), colorWaterMark);
            else
                getMc().fontRendererObj.drawStringWithShadow(MODECASE.getValue() == casemode.UPPER ? ("Healing Pots: " +  getPotions()).toUpperCase() : (MODECASE.getValue() == casemode.LOWER ? ("Healing Pots: " + getPotions()).toLowerCase() : ("Healing Pots: " + getPotions())), 3, event.getScaledResolution().getScaledHeight() - (getMc().ingameGUI.getChatGUI().getChatOpen() ? 24 : 12), colorWaterMark);
        }
    }
    ...
    private void drawNotifications(ScaledResolution scaledResolution) {
        ...
        for (int i = 0; i < Wool.INSTANCE.getNotificationManager().getNotifications().size(); i++) {
            Wool.INSTANCE.getNotificationManager().getNotifications().get(i).draw(y);
            y -= 26;
        }
    }

    private void drawArraylist() {
        for (Module module : Wool.INSTANCE.getModuleManager().getModuleMap().values()) {
            if (!module.isEnabled() || module.isHidden() || Wool.INSTANCE.getArraylistManager().isArrayMember(module))
                continue;
            Wool.INSTANCE.getArraylistManager().addArray(module);
        }
        float posY = 3;
        final ArrayList<ArrayMember> arrayMembers = new ArrayList<>(Wool.INSTANCE.getArraylistManager().getArrayMembers());
    ...
    private String getModuleRenderString(Module module) {
        if (MODECASE.getValue() == casemode.LOWER)
            return (Objects.nonNull(module.getRenderLabel()) ? (module.getRenderLabel() + (Objects.nonNull(module.getSuffix()) ? " " + module.getSuffix() : "")) : (module.getLabel() + (Objects.nonNull(module.getSuffix()) ? " " + module.getSuffix() : ""))).toLowerCase();
        else if (MODECASE.getValue() == casemode.UPPER)
            return (Objects.nonNull(module.getRenderLabel()) ? (module.getRenderLabel() + (Objects.nonNull(module.getSuffix()) ? " " + module.getSuffix() : "")) : (module.getLabel() + (Objects.nonNull(module.getSuffix()) ? " " + module.getSuffix() : ""))).toUpperCase();
        else
