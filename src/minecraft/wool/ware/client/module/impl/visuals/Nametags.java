...
import wool.ware.client.module.impl.combat.AntiBot;
...
import wool.ware.client.utils.value.impl.ColorValue;
import wool.ware.client.utils.value.impl.EnumValue;
import wool.ware.client.utils.value.impl.FontValue;
...
public class Nametags extends Module {
    ...
    public void drawPotions(EntityPlayer player, double posX, double posY) {
        ...
        for (Object o : player.getActivePotionEffects()) {
            ...
            if (potion != null && player.isPotionActive(potion) && potRanOut) {
                ...
                if (font.isEnabled())
                    fontValue.getValue().drawStringWithShadow(I18n.format(potion.getName()) + effectpower + ": " + Potion.getDurationString(effect), posX - fontValue.getValue().getStringWidth(I18n.format(potion.getName()) + effectpower + ": " + Potion.getDurationString(effect)) / 2, offset, potion.getLiquidColor());
                else {
                    GL11.glPushMatrix();
                    GL11.glScalef(0.5f, 0.5f, 0.5f);
                    getMc().fontRendererObj.drawStringWithShadow(I18n.format(potion.getName()) + effectpower + ": " + Potion.getDurationString(effect), (float) ((posX - getMc().fontRendererObj.getStringWidth(I18n.format(potion.getName()) + effectpower + ": " + Potion.getDurationString(effect)) / 4) * 2), (float) (offset * 2), potion.getLiquidColor());
                    GL11.glPopMatrix();
                    GL11.glScalef(1.0f, 1.0f, 1.0f);
                }
    ...
    private int getNameColor(EntityLivingBase ent) {
        if (Wool.INSTANCE.getFriendManager().isFriend(ent.getName())) return new Color(122, 190, 255).getRGB();
