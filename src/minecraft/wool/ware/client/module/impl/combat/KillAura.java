package wool.ware.client.module.impl.combat;
...
public class KillAura extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        final Criticals criticals = (Criticals) Wool.INSTANCE.getModuleManager().getModule("criticals");
        ...
        switch (Mode.getValue()) {
            ...
            case HYPIXEL:
            	 target = getTarget();
                 int attackKey1 = getMc().gameSettings.keyBindAttack.getKeyCode();
                 int blockKey1 = getMc().gameSettings.keyBindUseItem.getKeyCode();
                 if (event.isPre()) {
                     if (target != null) {
                    	  final float[] rots = getRotationsToEnt(target, getMc().thePlayer);
                          event.setYaw(rots[0]);
            ...
            case HVH:
                  ...
                  if (event.isPre()) {
                      ...
                      if (target != null) {
                              ...
                              if (target.hurtResistantTime == 0) {
                                  if (timerUtil.sleep(ping * 3)) attackEntity(target, true);
                              } else if (target.hurtResistantTime <= 9 + pingDelay) attackEntity(target, true);
                          } else if (timerUtil.sleep(900 / getCPS())) attackEntity(target, true);
                      timerUtil.reset();
                      if (canBlock() && nearbyTargets(true))
                    	  getMc().playerController.sendUseItem(getMc().thePlayer, getMc().theWorld, getMc().thePlayer.getHeldItem());
                        ...
                        if (isValidTicks(target)) {
                            if (!dura.isEnabled()) {
                                ...
                                if (helm != null) {
                                    ...
                                    Printer.print("Âf" + target.getName() + "Â7's Helmet Dura: Âf" + ey + " (" + damagedone + ")");
                                }
                                target.auraticks = 10;
                            }
                        }
    ...
    private boolean canCrit() {
        return getMc().thePlayer.onGround && !Wool.INSTANCE.getModuleManager().getModule("speed").isEnabled() && !getMc().gameSettings.keyBindJump.isKeyDown() && getMc().thePlayer.fallDistance == 0;
    }
    ...
    private enum mode {
        SINGLE, SWITCH, TICK, SMOOTH, CUBECRAFT, AAC, EXPERIMENTAL, MULTI, HVH, HYPIXEL
    }
}
