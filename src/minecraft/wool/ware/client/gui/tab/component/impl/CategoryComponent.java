package wool.ware.client.gui.tab.component.impl;

import net.minecraft.client.gui.ScaledResolution;
import wool.ware.client.Wool;
import wool.ware.client.gui.tab.TabGUI;
import wool.ware.client.gui.tab.component.Component;
...
public class CategoryComponent extends Component {
    ...
    public CategoryComponent(TabGUI mainTab, Module.Category category, float posX, float posY, float width, float height) {
        ...
        selectedModule = Wool.INSTANCE.getModuleManager().getModulesInCategory(category).isEmpty() ? null : Wool.INSTANCE.getModuleManager().getModulesInCategory(category).get(0);
    }

    @Override
    public void init() {
        float y = getPosY();
        for (Module module : Wool.INSTANCE.getModuleManager().getModulesInCategory(category)) {
            moduleComponents.add(new ModuleComponent(this, module, getPosX() + getWidth() + 2, y, Wool.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22, 12));
            y += 12;
        }
    ...
    public void onDraw(ScaledResolution scaledResolution) {
        for (ModuleComponent moduleComponent : moduleComponents) {
            ...
            moduleComponent.setWidth(Wool.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22);
        }
        ...
        if (mainTab.isExtendedModule() && mainTab.getSelectedCategory() == getCategory()) {
            ...
            RenderUtil.drawBorderedRect(getPosX() + getWidth() + 2, getPosY(), Wool.INSTANCE.getModuleManager().getLongestModInCategory(category) + 22, 12 * moduleComponents.size(), 0.5f, 0xff000000, 0x00000000);
        }
    }
            ...
            switch (key) {
                case Keyboard.KEY_DOWN:
                    if (!mainTab.isExtendedValue() && !mainTab.isExtendedValueDynamic())
                        setSelectedModule(Wool.INSTANCE.getModuleManager().getModulesInCategory(category).get((Wool.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) + 1) % Wool.INSTANCE.getModuleManager().getModulesInCategory(category).size()));
                    break;
                case Keyboard.KEY_UP:
                    if (!mainTab.isExtendedValue() && !mainTab.isExtendedValueDynamic())
                        setSelectedModule(Wool.INSTANCE.getModuleManager().getModulesInCategory(category).get((Wool.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) - 1) < 0 ? Wool.INSTANCE.getModuleManager().getModulesInCategory(category).size() - 1 : (Wool.INSTANCE.getModuleManager().getModulesInCategory(category).indexOf(getSelectedModule()) - 1)));
                    break;
