...
import wool.ware.client.gui.MainMenu;
import wool.ware.client.gui.account.gui.GuiAltManager;
import wool.ware.client.gui.account.gui.thread.AccountLoginThread;
import wool.ware.client.gui.account.system.Account;
import wool.ware.client.utils.thealtening.TheAltening;
import wool.ware.client.utils.thealtening.domain.AlteningAlt;
...
public class GuiDisconnected extends GuiScreen {
    ...
    public void initGui() {
        ...
        if (Wool.INSTANCE.isAutoRelogNormal() || Wool.INSTANCE.isAutoRelogTheAltening())
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 4) {
            if (GuiAltManager.currentAccount != null) {
                ...
                Wool.INSTANCE.getAccountManager().getAccounts().remove(GuiAltManager.currentAccount);
                Wool.INSTANCE.getAccountManager().save();
            }
        }
        if (button.id == 5) {
            if (Wool.INSTANCE.getAccountManager().getAccounts().isEmpty()) return;
            ArrayList<Account> registry = Wool.INSTANCE.getAccountManager().getAccounts();
            Random random = new Random();
            Account randomAlt = registry.get(random.nextInt(Wool.INSTANCE.getAccountManager().getAccounts().size()));
        ...
        if (button.id == 6) {
            if (Wool.INSTANCE.getAccountManager().getAlteningKey() == null) return;
            niggaButton = true;
            try {
                TheAltening theAltening = new TheAltening(Wool.INSTANCE.getAccountManager().getAlteningKey());
        ...
        if (button.id == 8) {
            if (Wool.INSTANCE.getAccountManager().getAccounts().isEmpty()) return;
            Wool.INSTANCE.setAutoRelogNormal(!Wool.INSTANCE.isAutoRelogNormal());
            Wool.INSTANCE.setAutoRelogTheAltening(false);
            reconnectTime = System.currentTimeMillis() + 1500;
        }
        if (button.id == 9) {
            if (Wool.INSTANCE.getAccountManager().getAlteningKey() == null) return;
            Wool.INSTANCE.setAutoRelogTheAltening(!Wool.INSTANCE.isAutoRelogTheAltening());
            Wool.INSTANCE.setAutoRelogNormal(false);
            reconnectTime = System.currentTimeMillis() + 1500;
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.multilineMessage != null) {
            for (Iterator var5 = this.multilineMessage.iterator(); var5.hasNext(); var4 += this.fontRendererObj.FONT_HEIGHT) {
                ...
                drawCenteredString(fontRendererObj, "Auto Relog (Normal): " + Wool.INSTANCE.isAutoRelogNormal(), width / 2, GuiAltManager.currentAccount != null ? 34 : 20, -1);
                drawCenteredString(fontRendererObj, "Auto Relog (TheAltening): " + Wool.INSTANCE.isAutoRelogTheAltening(), width / 2, GuiAltManager.currentAccount != null ? 48 : 34, -1);
            }
        }
        if (Wool.INSTANCE.isAutoRelogNormal() || Wool.INSTANCE.isAutoRelogTheAltening()) {
            ...
            if (System.currentTimeMillis() >= reconnectTime) {
                if (Wool.INSTANCE.isAutoRelogTheAltening()) {
                    if (Wool.INSTANCE.getAccountManager().getAlteningKey() == null) return;
                    niggaButton = true;
                    try {
                        TheAltening theAltening = new TheAltening(Wool.INSTANCE.getAccountManager().getAlteningKey());
                        ...
                        Wool.INSTANCE.getAccountManager().setLastAlteningAlt((Objects.requireNonNull(account).getToken().replaceAll(" ", "")));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                } else {
                    ...
                    try {
                        (GuiAltManager.loginThread = new AccountLoginThread(user2, pass2)).start();
                        Wool.INSTANCE.getAccountManager().setLastAlt(randomAlt);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
