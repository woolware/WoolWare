...
import wool.ware.client.module.impl.combat.KillAura;
import wool.ware.client.module.impl.other.MurderMystery;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                if (entity == Minecraft.getMinecraft().thePlayer) {
                    if (Wool.INSTANCE.getModuleManager().getModule("killaura").isEnabled() && KillAura.target != null || Wool.INSTANCE.getModuleManager().getModule("antiaim").isEnabled() || Wool.INSTANCE.getModuleManager().getModule("scaffold").isEnabled()) {
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
        ...
        final Color visible = MurderMystery.murderers.contains(entitylivingbaseIn) ? new Color(255,0,0,255) : (new Color((Wool.INSTANCE.getFriendManager().isFriend(entitylivingbaseIn.getName()) || MurderMystery.gudpeople.contains(entitylivingbaseIn)) ? chams.visiblefriend.getValue() :chams.visible.getValue()));
        final Color hidden =MurderMystery.murderers.contains(entitylivingbaseIn) ? new Color(255, 128, 128,255) : (new Color((Wool.INSTANCE.getFriendManager().isFriend(entitylivingbaseIn.getName()) || MurderMystery.gudpeople.contains(entitylivingbaseIn)) ? chams.hiddenfriend.getValue() : chams.hidden.getValue()));
        ...
        if (flag || flag1) {
            ...
            if (Wool.INSTANCE.getModuleManager().getModuleClass(Chams.class).isEnabled() && chams.isValid(entitylivingbaseIn) && chams.colored.isEnabled()) {
